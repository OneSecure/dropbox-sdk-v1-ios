...
-(void) loader:(id)obj1 didFailWithError:(id)obj2 {
    NSLog(@"Make compiler happy.");
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
	...
	if (_action) {
		...
		if ([_target conformsToProtocol:@protocol(MPOAuthAPIInternalClient)]) {
            func(_target, _action, self, self.data);
		} else {
            func(_target, _action, self.oauthRequest.url, self.responseString);
		}
	}
}
